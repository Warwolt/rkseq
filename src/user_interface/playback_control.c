...
void playback_control_update(ui_devices_t* ui_devices, beat_clock_t* beat_clock) {
	...
	/* Update Tempo */
	const int rotary_diff = rotary_encoder_read(&ui_devices->tempo_knob);
	beat_clock_set_tempo(beat_clock, beat_clock->tempo_bpm + rotary_diff);

	/* Display Current Tempo*/
	segment_display_set_number(&ui_devices->tempo_display, beat_clock->tempo_bpm * 10);
	segment_display_enable_period(&ui_devices->tempo_display, 1);
	segment_display_update(&ui_devices->tempo_display);
}
